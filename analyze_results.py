...
def main() -> None:
    ...
    for row in rows:
        key = (row["reward"], row["exploration"], row["architecture"])
    ...
    for key, values in sorted(groups.items()):
        summary = {"reward": key[0], "exploration": key[1], "architecture": key[2],
                   "n_seeds": len(values)}
    ...
    fig.suptitle("Exploration × reward shaping: reward-independent evaluation")
    fig.tight_layout()
