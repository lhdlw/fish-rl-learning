...
比较稀疏奖励、距离奖励塑形与随机策略。重点是**可核查的实验与失败分析**。

**当前结论：在本次小预算实验中，两种 DQN 的平均吃鱼数均未超过随机策略。**
最开始有优于，后期越修改，越调参反而更坏了，可以当成一个失败案例来看吧orz。

## 1. 问题与本次实验

最开始直接比较不同奖励函数下的训练分数，后面发现分数尺度不同，不能据此判断谁更会吃鱼。
后来把“学习用的奖励”和“统一的任务评估”分开，并增加无需训练的随机策略。
